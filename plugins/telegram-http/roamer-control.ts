...
export async function connectToSession(
  ...
  // Need to respawn with --channels. If existing tmux, kill it first.
  if (session.tmuxName) {
  ...
  if (!spawned.ok) {
    const m = `❌ 無法 spawn tmux + claude（缺 TELEGRAM_HTTP_PORT env？）`
    await notify(m)
    return { ok: false, message: m }
  }
...
async function handleChannelBotCommandsForCurrentTarget(
  ...
  // /restart — kill current tmux + force user to /roam again to re-takeover.
  if (isRestart) {
    try {
  ...
  // /kill_stuck — SIGKILL current target's claude PID.
  if (isKillStuck) {
    try {
      if (target.claude_pid && pidAlive(target.claude_pid)) {
        process.kill(target.claude_pid, 'SIGKILL')
        await replyToTg(`⛔ 已 kill -9 當前 target claude (PID ${target.claude_pid}). tmux 仍存在，重新 /roam 該 project 接管。`)
      } else {
